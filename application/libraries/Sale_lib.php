...
class Sale_lib
{
	...
	public function get_totals($taxes)
	{
		...
		foreach($this->get_cart() as $item)
		{
			...
			$total_commission=bcadd($total_commission,$item['quantity']*$item['commission_price']);
		...
		$totals['total_commission']=$total_commission;
		$sales_tax = 0;
		...
		$additional_charges = $this->get_additional_charges();
		$totals['total'] = bcadd($totals['total'], $additional_charges);  
		$total = bcadd($total, $additional_charges); // Adding additional charges to total
	...
	// Method to set additional charges in session
	public function set_additional_charges($additional_charges) {
		$this->CI->session->set_userdata('additional_charges', $additional_charges);
	}

	// Method to get additional charges from session
	public function get_additional_charges() {
		return $this->CI->session->userdata('additional_charges') ? (float)$this->CI->session->userdata('additional_charges') : 0;
	}

	//Method to clear additional charge from session
	public function clear_additional_charges()
	{
		$this->CI->session->unset_userdata('additional_charges');
	}
	...
	public function add_item(&$item_id,$batch_number, $quantity = 1, $item_location, &$discount = 0.0, $discount_type = 0, $price_mode = PRICE_MODE_STANDARD, $kit_price_option = NULL, $kit_print_option = NULL, $price_override = NULL, $description = NULL, $serialnumber = NULL, $sale_id = NULL, $include_deleted = FALSE, $print_option = NULL, $line = NULL, $customer_category = 'Retail')
	{
		$item_info = $this->CI->Item->get_info_by_id_or_number($item_id, $batch_number,$include_deleted);
		...
		$mode = $this->get_mode();
		if($mode == 'sale_invoice' && $customer_category == 'Wholesale'){
		...
		$commission_percentage = $item_info->commission_percentage;
		$commission_price = $item_info->commission_price;
		...
		$attribute_links = $this->CI->Attribute->get_link_values($item_id,$batch_number, 'sale_id', $sale_id, Attribute::SHOW_IN_SALES)->row_object();
		...
		if(!$itemalreadyinsale || $item_info->is_serialized)
		{
			$item = array($insertkey => array(
					...
					'in_stock' => $this->CI->Item_quantity->get_item_quantity($item_id, $batch_number, $item_location)->quantity,
					'price' => $price,
	...
	public function out_of_stock($item_id,$batch_number, $item_location)
	{
		//make sure item exists
		if($item_id != -1)
		{
			$item_info = $this->CI->Item->get_info_by_id_or_number($item_id,$batch_number);

			if($item_info->stock_type == HAS_STOCK)
			{
				$item_quantity = $this->CI->Item_quantity->get_item_quantity($item_id, $batch_number, $item_location)->quantity;
	...
	public function edit_item($line,$batch_number,$description, $serialnumber, $quantity, $discount, $discount_type, $price, $discounted_total=NULL)
	{
		$items = $this->get_cart();
		if(isset($items[$line]))
		{
	...
	public function return_entire_sale($receipt_sale_id)
	{
		...
		{
			$this->add_item($row->item_id,$row->batch_number,-$row->quantity_purchased, $row->item_location, $row->discount, $row->discount_type, PRICE_MODE_STANDARD, NULL, NULL, $row->item_unit_price, $row->description, $row->serialnumber, NULL, TRUE);
		}
	...
	public function copy_entire_sale($sale_id)
	{
		...
		foreach($this->CI->Sale->get_sale_items_ordered($sale_id)->result() as $row)
		{
			$this->add_item($row->item_id, $row->batch_number, $row->quantity_purchased, $row->item_location, $row->discount, $row->discount_type, PRICE_MODE_STANDARD, NULL, NULL, $row->item_unit_price, $row->description, $row->serialnumber, $sale_id, TRUE, $row->print_option);
		}
