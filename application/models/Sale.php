...
class Sale extends CI_Model
{
    ...
    // Functions for token functionality
	public function set_token($token_number=1)
	{
		$data=array('value'=> $token_number);
	    $this->db->where('key', 'token_number'); 
	...
	public function get_token()
	{
		...
		if ($query->num_rows() > 0) {
			return $query->row()->value; // Return the value
		}
		...
		foreach($items as $line=>$item)
		{
			...
			if($cur_item_info->stock_type == HAS_STOCK && $sale_status == COMPLETED)
			{
				...
				$this->Item_quantity->save(array('quantity'	=> $item_quantity->quantity - $item['quantity'],
					'item_id'		=> $item['item_id'],
					'location_id'	=> $item['item_location']), $item['item_id'],$item['batch_number'], $item['item_location']);
			...
			$this->Attribute->copy_attribute_links($item['item_id'],$item['batch_number'],'sale_id', $sale_id);
		}
		...
		if($update_inventory && $sale_status == COMPLETED)
		{
			...
			foreach($items as $item)
			{
				...
				if($cur_item_info->stock_type == HAS_STOCK)
				{
					...
					// update quantities
					$this->Item_quantity->change_quantity($item['item_id'], $item['batch_number'],$item['item_location'], $item['quantity_purchased']);
				}
			}
		}
	...
	public function get_sale_items_ordered($sale_id)
	{
		...
		$this->db->join('batches AS batches', 'sales_items.item_id = batches.item_id and(items.batch_number=sales_items.batch_number or sales_items.batch_number=batches.batch_number) and items.batch_number!=batches.batch_number','left');
