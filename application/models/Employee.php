...
class Employee extends Person
{
	...
	*/
	...
	public function save_employee_salary($employees_advance_salary,$employees_daily_salary, $employees_monthly_salary, $employee_id)
	{
		$success = FALSE;
		...
		{
			$employee_data['employee_advance_salary'] = $employees_advance_salary;
			$employee_data['employee_daily_salary'] = $employees_daily_salary;
			$employee_data['employee_monthly_salary'] = $employees_monthly_salary;
			if(!$employee_id || !$this->exists_employee_salary($employee_id))
			{
				$employee_data['person_id'] = $employee_id;
				$success = $this->db->insert('employee_salary', $employee_data);
			}
			else
			{
				$this->db->where('person_id', $employee_id);
				$success = $this->db->update('employee_salary', $employee_data);
			}
...
/*
	Gets information about a particular employee
	*/
	public function get_employee_and_salary_info($employee_id)
	{
		$this->db->select('CONCAT(first_name, " ",last_name) AS full_name, employee_daily_salary, employee_advance_salary');
		$this->db->from('employees AS employees');
		$this->db->join('people', 'employees.person_id = people.person_id');
		$this->db->join('employee_salary', 'employees.person_id = employee_salary.person_id', 'left');
		$this->db->where('employees.person_id', $employee_id);
	...
	public function save_employee(&$person_data, &$employee_data, &$grants_data, $employee_id = FALSE)
	{
		...
		if(ENVIRONMENT != 'testing' && parent::save($person_data, $employee_id))
		{
			if(!$employee_id || !$this->exists($employee_id))
			{
				...
				if ($success) {
					$new_employee_id = $this->db->insert_id(); // Get the ID of the newly inserted employee
					$this->save_employee_salary(0,0,0,$employee_data['person_id']);
				} 
			}
			else
			{
