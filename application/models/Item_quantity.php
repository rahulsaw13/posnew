...
{
    public function exists($item_id,$batch_number, $location_id)
    {
    ...
    public function save($location_detail, $item_id, $batch_number,$location_id)
    {
        if(!$this->exists($item_id,$batch_number, $location_id))
        {
    ...
    public function get_item_quantity($item_id,$batch_number, $location_id)
    {
        $this->db->from('item_quantities');
        $this->db->where('item_id', $item_id);
        $this->db->where('batch_number',$batch_number);
    ...
    public function get_batch_quantity($batch_id)
    {
    ...
    public function update_batch_quantity($batch_id, $quantity)
    {
        $this->db->where('id', $batch_id);
	 ...
	 */
	public function change_quantity($item_id,$batch_number, $location_id, $quantity_change)
	{
		$quantity_old = $this->get_item_quantity($item_id,$batch_number, $location_id);
		$quantity_new = $quantity_old->quantity + $quantity_change;
		$location_detail = array('item_id' => $item_id,'batch_number'=>$batch_number, 'location_id' => $location_id, 'quantity' => $quantity_new);

		return $this->save($location_detail, $item_id,$batch_number, $location_id);
	}

	/*
	* Set to 0 all quantity in the given item
	*/
	public function reset_quantity($item_id,$batch_number)
	{
        $this->db->where('item_id', $item_id);
        $this->db->where('batch_number', $batch_number);
