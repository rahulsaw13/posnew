...
class Customer extends Person
{
	...
	*/
    ...
    // Fetch customer category options (Retail, Wholesale)
    public function get_customer_categories()
    {
        // Return predefined options for customer category
        return array(
            'Retail' => 'Retail',
            'Wholesale' => 'Wholesale'
        );
	...
	*/
	public function get_infos($customer_id)
	{$this->db->select("customers.person_id AS customer_id, CONCAT(first_name, ' ', last_name) AS name");
		$this->db->from('customers');
		$this->db->join('people', 'people.person_id = customers.person_id');
		$this->db->where_in('customers.person_id', $customer_id);
		$res = $this->db->get()->result();
		
		// Map the result into an array with customer_id as key and name as value
		$customer_name_map = array();
		foreach ($res as $row) {
			$customer_name_map[$row->customer_id] = (string) $row->name;
		}
		
		return $customer_name_map;
	}
	...
	*/
	public function get_stats($customer_id)
	{
		...
		// Get totals and include credit note directly from the customer table
		$this->db->select('
						...
						customers.credit_note AS credit_note
						");
		$this->db->from('sales');
		...
		// Join with the customers table to get the credit note value
		$this->db->join('customers AS customers', 'sales.customer_id = customers.person_id', 'left');
	...
	*/
	...
	public function update_credit_note_balance($customer_id, $new_balance)
	{
		$this->db->where('person_id', $customer_id);
		return $this->db->update('customers', array('credit_note' => $new_balance));
	}
