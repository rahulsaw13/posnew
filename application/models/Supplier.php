...
class Supplier extends Person
{
	...
	*/
	public function get_infos($supplier_id)
	{$this->db->select("suppliers.person_id AS supplier_id, CONCAT(first_name, ' ', last_name) AS name");
		$this->db->from('suppliers');
		$this->db->join('people', 'people.person_id = suppliers.person_id');
		$this->db->where_in('suppliers.person_id', $supplier_id);
		$res = $this->db->get()->result();
		
		// Map the result into an array with customer_id as key and name as value
		$supplier_name_map = array();
		foreach ($res as $row) {
			$supplier_name_map[$row->supplier_id] = (string) $row->name;
		}
