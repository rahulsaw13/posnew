...
class Attribute extends CI_Model
{
	...
	public function get_latest_batch_number($item_id)
	{
		$this->db->select('items.batch_number');
		$this->db->from('items');
		$this->db->where('items.item_id', $item_id);
		$result = $this->db->get()->row();
    // Check if result is not empty and return the batch_number as a string
    return !empty($result) ? (string)$result->batch_number : '';
	}
	 ...
	 */
	public function link_exists($item_id,$batch_number, $definition_id = FALSE)
	{
	...
	public function get_attributes_by_item($item_id,$batch_number)
	{
	...
	private function checkbox_attribute_values($definition_id)
	{
		...
		{
			$zero_attribute_id = $this->save_value('0', $definition_id, FALSE,FALSE, FALSE, CHECKBOX);
		}

		if($one_attribute_id === FALSE)
		{
			$one_attribute_id = $this->save_value('1', $definition_id, FALSE, FALSE,FALSE, CHECKBOX);
		}
	...
	public function save_definition(&$definition_data, $definition_id = NO_DEFINITION_ID)
	{
		...
		if($definition_id === NO_DEFINITION_ID || !$this->exists($definition_id))
		{
			...
			{
				$success = $this->Attribute->undelete_definition($definition_id);
			}
			else
			{
	...
	public function save_link($item_id,$batch_number, $definition_id, $attribute_id)
	{
		$this->db->trans_start();
	...
	public function delete_link($item_id,$batch_number,$definition_id = FALSE)
	{
		$delete_data = array('item_id' => $item_id);
		$delete_data = array('batch_number' => $batch_number);
	...
	public function get_link_value($item_id,$batch_number, $definition_id)
	{
		$this->db->where('item_id', $item_id);
		$this->db->where('batch_number', $batch_number);
		$this->db->where('sale_id');
	...
	public function get_link_values($item_id,$batch_number, $sale_receiving_fk, $id, $definition_flags)
	{
	...
	public function get_attribute_value($item_id, $batch_number,$definition_id)
	{
	...
	public function get_attribute_values($item_id,$batch_number)
	{
	...
	public function copy_attribute_links($item_id, $batch_number, $sale_receiving_fk, $id)
	{
		$this->db->query(
			'INSERT INTO ' . $this->db->dbprefix('attribute_links') . ' (item_id, definition_id,batch_number, attribute_id, ' . $sale_receiving_fk . ')
			SELECT ' . $this->db->escape($item_id) . ', definition_id,' . $this->db->escape($batch_number) . ', attribute_id,' . $this->db->escape($id) . '
			FROM ' . $this->db->dbprefix('attribute_links') . '
			WHERE item_id = ' . $this->db->escape($item_id) . ' AND  '.'batch_number =' . $this->db->escape($batch_number) . ' AND sale_id IS NULL AND receiving_id IS NULL'
			);
	}
	...
	public function save_value($attribute_value, $definition_id, $item_id = FALSE,$batch_number = FALSE, $attribute_id = FALSE, $definition_type = DROPDOWN)
	{
		$this->db->trans_start();
	...
	*/
	...
	public function attribute_cleanup($attributes, $definition_id, $definition_type)
	{
		...
		{
			$new_attribute_id = $this->save_value($attribute['attribute_value'], $definition_id, FALSE,FALSE, $attribute['attribute_id'], $definition_type);

			if($this->save_link($attribute['item_id'], $attribute['batch_number'],$definition_id, $new_attribute_id) == FALSE)
			{
	...
	{
		$this->db->select('attribute_links.attribute_id, item_id, batch_number,attribute_value, attribute_decimal, attribute_date');
		$this->db->join('attribute_values', 'attribute_values.attribute_id = attribute_links.attribute_id');
		$this->db->where('definition_id', $definition_id);
