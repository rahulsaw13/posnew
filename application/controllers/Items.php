...
class Items extends Secure_Controller
{
	...
	public function view_item($item_id = NEW_ITEM)
	{
		...
		foreach($stock_locations as $location)
		{
			...
			$quantity = $this->xss_clean($this->Item_quantity->get_item_quantity($item_id,$batch_number,$location['location_id'])->quantity);
	...
	public function view($item_id = NEW_ITEM)
	{
		...
		$batch_number=$this->Attribute->get_latest_batch_number($item_id);
		...
		$data['definition_values'] = $this->Attribute->get_attributes_by_item($item_id,$batch_number);
	...
	// code added by kevin
	public function view_payment($item_id = NEW_ITEM) {
		if($item_id === NEW_ITEM)
		{
			$data = [];
		}
		...
		$this->load->view('items/payment_form', $data);
	}

	// public function fake_view($item_id = NEW_ITEM)
	// {
	// 	if($item_id === NEW_ITEM)
	// 	{
	// 		$data = [];
	...
	// 	$data['batch'] = $this->Item->get_batch_by_item_id($item_id);
	// 	foreach(get_object_vars($data['batch']) as $property => $value)
	// 	{
	// 	    $data['batch']->$property = $this->xss_clean($value);
	// 	}
	...
	public function attributes($item_id = NEW_ITEM)
	{
		...
		$data['definition_values'] = $this->Attribute->get_attributes_by_item($item_id,$batch_number) + $this->Attribute->get_values_by_definitions($definition_ids);
		$data['definition_names'] = $this->Attribute->get_definition_names();
		$data['batch_number'] =$this->Attribute->get_latest_batch_number($item_id);

		foreach($data['definition_values'] as $definition_id => $definition_value)
		{
			$attribute_value = $this->Attribute->get_attribute_value($item_id, $data['batch_number'],$definition_id);
...
//             {
//                 $attribute_id = $this->Attribute->save_value($attribute_id, $definition_id, $item_id,$batch_number $attribute_ids[$definition_id], $definition_type);
//             }
...
//             'cost_price' => parse_decimals($this->input->post('batch_cost_price')),
//             'unit_price' => parse_decimals($this->input->post('batch_unit_price')),
//             'expiry_date' => $this->input->post('batch_expiry_date'),
//             'manufacture_date' => $this->input->post('batch_manufacture_date'),
//             'quantity' => parse_quantity($this->input->post('batch_quantity')),
//             'reorder_level' => parse_quantity($this->input->post('batch_reorder_level')),
//             'item_id' => $item_id
//         );
//         $success &= $this->Item->save_batch($batch_data);
...
// }
	...
	public function save($item_id = NEW_ITEM)
	{
		...
		$item_data = array(
			...
			// 'commission_price' => parse_decimals($this->input->post('commission_price')),
			// 'commission_percentage' => parse_decimals($this->input->post('commission_percentage')),
			'reorder_level' => parse_quantity($this->input->post('reorder_level')),
			'receiving_quantity' => $receiving_quantity,
			'allow_alt_description' => $this->input->post('allow_alt_description') !== NULL,
			'is_serialized' => $this->input->post('is_serialized') !== NULL,
		...
		if($this->Item->save($item_data, $item_id))
		{
			...
			foreach($stock_locations as $location)
			{
				...
				$item_quantity = $this->Item_quantity->get_item_quantity($item_id,$batch_number, $location['location_id']);

				if($item_quantity->quantity != $updated_quantity || $new_item)
				{
					$success &= $this->Item_quantity->save($location_detail, $item_id, $batch_number, $location['location_id']);
			...
			$res=$this->Attribute->delete_link($item_id,$batch_number); 
			...
			foreach($attribute_links as $definition_id => $attribute_id)
			{
				...
				{
					$attribute_id = $this->Attribute->save_value($attribute_id, $definition_id, $item_id,$batch_number,$attribute_ids[$definition_id], $definition_type);
					$this->Attribute->save_link($item_id,$batch_number,$definition_id,$attribute_id);
				}
	...
	public function save_inventory($item_id = NEW_ITEM)
	{
		...
		$batch_number=$cur_item_info->batch_number;
		$inv_data = array(
			'trans_date' => date('Y-m-d H:i:s'),
			'trans_items' => $item_id,
			'trans_user' => $employee_id,
	...
	//Update stock quantity
		$item_quantity = $this->Item_quantity->get_item_quantity($item_id, $batch_number,$location_id);
		...
		if($this->Item_quantity->save($item_quantity_data, $item_id,$batch_number, $location_id))
		{
		...
		else
		{
			if(file_exists($_FILES['file_path']['tmp_name']))
			{
				...
				foreach($csv_rows as $key => $row)
				{
					...
					// // Fetch the commission percentage and cost price from the CSV row
					// $commission_percentage = isset($row['Commission %']) ? $row['Commission %'] : 0;
					// $cost_price = isset($row['Cost Price']) ? $row['Cost Price'] : 0;
					
					// // Calculate commission price if commission percentage and cost price are available
					// if ($commission_percentage > 0 && $cost_price > 0) {
					// 	$commission_price = ($commission_percentage / 100) * $cost_price;  
					// } else {
					// 	$commission_price = 0; // Default to 0 if commission percentage or cost price is missing
    				// }
					$item_data = array(
						'item_id' => $item_id,
						'name' => $row['Item Name'],
	...
	private function data_error_check($row, $item_data, $allowed_locations, $definition_names, $attribute_data)
	{
		...
		$check_for_numeric_values = array(
			...
			'unit_price' => $item_data['unit_price'],
			// 'wholesale_price' => $item_data['wholesale_price'],
			// 'commission_price' => $item_data['commission_price'],
		...
		foreach($definitions as $definition)
		{
			...
			if(!empty($attribute_value) || $attribute_value === '0')
			{
				if($definition['definition_type'] === CHECKBOX)
				{
					...
					$attribute_id = $this->store_attribute_value($attribute_value, $definition, $item_data['item_id'],$item_data['batch_number']);
				}
	 ...
	 */
	private function store_attribute_value($value, $attribute_data, $item_id,$batch_number)
	{
		...
		if($attribute_id === FALSE)
		{
			$attribute_id = $this->Attribute->save_value($value, $attribute_data['definition_id'], $item_id, $batch_number,FALSE, $attribute_data['definition_type']);
		}
		else if($this->Attribute->save_link($item_id, $batch_number,$attribute_data['definition_id'], $attribute_id) === FALSE)
		{
			return FALSE;
		}
		return $attribute_id;
	...
	private function save_inventory_quantities($row, $item_data, $allowed_locations, $employee_id)
	{
		...
		foreach($allowed_locations as $location_id => $location_name)
		{
			...
			if(!empty($row["location_$location_name"]) || $row["location_$location_name"] === '0')
			{
				...
				$this->Item_quantity->save($item_quantity_data, $item_data['item_id'],$item_data['batch_number'], $location_id);
