...
class Sales extends Secure_Controller
{
    ...
    public function send_whatsapp_receipt($sale_id)
    {
        $inputs = $this->input->post();
        
        // Example of how you can get the customer’s phone number from the sale data
        $customer_phone = 9852857975;  // Replace with actual retrieval logic
        
        // Generate receipt link (you can adjust the logic as per your system)
        $receipt_url = site_url('sales/receipt/' . $sale_id);  // Assuming this generates the receipt

        // Send the receipt via WhatsApp using Twilio
        $this->load->library('twilio_lib');  // Load Twilio or any WhatsApp messaging service
        try {
            $response = $this->twilio_lib->send_whatsapp_message(
                'whatsapp:' . $customer_phone,   // Recipient
                'whatsapp:+14155238886',    // Your Twilio WhatsApp Number
                'Here is your receipt for sale ID: ' . $sale_id,   // Message Body
                $receipt_url                     // Media URL (The receipt link or PDF URL)
            );
	...
	public function search_item()
	{
		...
		$items = $this->Item->get_items_with_batches($search);
	...
	public function item_search()
	{
		...
		$suggestions = array_merge($suggestions, $this->Item->get_search_suggestions_with_details($search, array('search_custom' => FALSE, 'is_deleted' => FALSE), TRUE));
	...
	{
		$data = array();
		$cumm_data=$this->input->post();
		$cumm_data=$cumm_data['item'];
		$pattern = '/Batch: (\d+), id: (\d+)/';
		$batch="";
		$id="";
		// Perform the regular expression match
		if (preg_match($pattern, $cumm_data, $matches)) {
			// $matches[1] contains the batch number
			$batch = $matches[1];
		...
		elseif($this->Item_kit->is_valid_item_kit($item_id_or_number_or_item_kit_or_receipt))
		{
			...
			{
				if(!$this->sale_lib->add_item($kit_item_id,$batch_number, $quantity, $item_location, $discount, $discount_type, PRICE_MODE_KIT, $kit_price_option, $kit_print_option, $price))
				{
		...
		{
			if(!$this->sale_lib->add_item($item_id_or_number_or_item_kit_or_receipt, $batch_number,$quantity, $item_location, $discount, $discount_type, PRICE_MODE_STANDARD, NULL, NULL, $price))
			{
				$data['error'] = $this->lang->line('sales_unable_to_add_item');
			}
			else
			{
				$data['warning'] = $this->sale_lib->out_of_stock($item_id_or_number_or_item_kit_or_receipt,$batch_number, $item_location);
			}
		}
		$this->_reload($data);
	}
	...
	public function edit_item($item_id)
	{
		...
		if($this->form_validation->run() != FALSE)
		{
			$this->sale_lib->edit_item($item_id,$batch_number,$description, $serialnumber, $quantity, $discount, $discount_type, $price, $discounted_total);
		...
		$data['warning'] = $this->sale_lib->out_of_stock($this->sale_lib->get_item_id($item_id),$batch_number, $item_location);
	...
	public function complete()
	{
		...
		//$this->send_whatsapp_receipt($sale_id);
	...
	// 					);
	// 					$this->Item->save_batch($batch_data);
	// 				}
	// 			}
	// 		$data = $this->xss_clean($data);
	...
	public function send_receipt($sale_id)
	{
		...
		if(!empty($sale_data['customer_phone_number']))
		{
			...
			//======
			$html = $this->load->view("sales/receipt_default", $sale_data, TRUE);
    
            // Load pdf helper
            $this->load->helper(array('dompdf', 'file'));
    
            // Set directory for saving PDF
            $directory = "/home/u677219137/domains/dukaandari.com/public_html/uploads/pointofsale";
            if (!is_dir($directory)) {
                mkdir($directory, 0755, true); // Create directory if it doesn't exist
            }
    
            // Generate the PDF filename
            $filename = $directory . '/' . $this->lang->line("sales_receipt") . '-' . str_replace('/', '-', $sale_data['sale_id']) . '.pdf';
    
            // Create the PDF and store it
            if (file_put_contents($filename, create_pdf($html)) !== FALSE) {
                // Generate the URL for the saved PDF
                $pdf_url = 'https://dukaandari.com/uploads/pointofsale/' . basename($filename);
                // Append the PDF link to the email text
                $text = ". \n\nYou can also download your invoice from here: " . $pdf_url;
                //$text =  $pdf_url;
                // Send the email with the PDF attachment
            $result = $this->sms_lib->sendsms($to, $text);
	...
	private function _reload($data = array())
	{
		...
		foreach ($items->result() as $item) { // Assuming $items->result() returns an array of objects
			$category = $item->category; // Access the category directly
			...
			// Simplify the item data to include only required fields
			$simplified_item = [
				'item_id' => $item->item_id, // Use the correct field name
				'name' => $item->name,
				'batch_number' => $item->batch_number,
				'cost_price' => $item->cost_price,
				'unit_price' => $item->unit_price,
				'quantity' => $item->receiving_quantity,
				'tax_percents' => $item->tax_category_id,
				'pic_filename' => $item->pic_filename
			];

			// Add the item to the respective category group
			$grouped_data[$category]['items'][] = $simplified_item;
			$all_items_data[] = $simplified_item;
		}
    ...
    {
        // Load the sale library to access cart and totals
        $this->load->library('sale_lib');
        
        // Get the cart items from the session
        $cart_items = $this->sale_lib->get_cart();
        
        // Get the taxes from the session or database (you should already have them available)
        // $taxes = $this->tax_lib->get_taxes($data['cart']);
        
        // Get the totals by calling get_totals() and passing the taxes
        $totals = $this->sale_lib->get_total();
        
        // Pass the cart items and totals to the view
        $data['cart_items'] = $cart_items;
        $data['totals'] = $totals;
    
        // Prepare grouped data (if necessary)
        $groupedData = [];
        foreach ($cart_items as $item) {
            $attributeValue = $item['attribute_values'];
            if (!isset($groupedData[$attributeValue])) {
                $groupedData[$attributeValue] = [];
                // Fetch and set the token number
                $token_no = ($this->Sale->get_token() == "null" ? 1 : $this->Sale->get_token());
                $this->Sale->set_token($token_no + 1);
            }
            $item['token_number'] = $token_no;
            $groupedData[$attributeValue][] = $item;
        }
        
        // Add grouped data to the view
        $data['groupedData'] = $groupedData;
        
        // Load the print_token view
        $this->load->view('sales/print_token', $data);
    }
